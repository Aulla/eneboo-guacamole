...
var GuacamoleUI = {
    ...
    "minombre"	  : document.getElementById("username"),
...
GuacamoleUI.attach = function(guac) {
    ...
    guac.onclipboard = function(data) {
    	if (data.indexOf("GUACA_IMPRIME") != -1) 
		{
		window.open(location.protocol+"//"+location.hostname+":"+location.port+"/guacamole/traspaso/"+data.substring(13));
		guac.setClipboard(""); //limpiamos el portapapeles local,para que cuando venga de nuevo reaccione correctamente
		GuacamoleUI.clipboard.value = ""
        	GuacamoleUI.touchClipboard.value = ""
		}
	else
		{
